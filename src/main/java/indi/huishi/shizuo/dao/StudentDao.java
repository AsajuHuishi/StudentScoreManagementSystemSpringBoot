package indi.huishi.shizuo.dao;
...
//    List<Student> showAll(@Param("begin") Integer begin, @Param("pageSize") Integer pageSize);
     ...
     * @return
     */
    @Select("select class_name as className, count(*) as countStudent, max(score) as maxScore, min(score) as minScore, round(avg(score),4) as avgScore from student_score group by class_name order by class_name")
